...
FACEBOOK_APP_ID = "169837426382293"
FACEBOOK_APP_SECRET = "d58a579b3d70a85a0816f190b1788bb1"
...
class BaseHandler(webapp.RequestHandler):
    ...
    def create_login_url(self, url):
        ...
        set_cookie(self.response, 'url', url)
        return parsed[0] + '://' + parsed[1]+"/facebook/login?"+urllib.urlencode({'continue':url})
    
    def create_logout_url(self, url):
        """aaa"""
        parsed = urlparse.urlparse(self.request.uri)
        set_cookie(self.response, 'url', url)
        return parsed[0] + '://' + parsed[1]+"/facebook/logout?"+urllib.urlencode({'continue':url})
...
class LoginHandler(BaseHandler):
    def get(self):
        ...
        args = dict(client_id=FACEBOOK_APP_ID, redirect_uri=self.request.path_url+"?"+urllib.urlencode({'continue':url}), scope='email')
        if self.request.get("code"):
            ...
            url = parse_cookie(self.request.cookies.get("continue"))
            set_cookie(self.response, "continue", "", expires=time.time() - 86400)
            if not url: url = '/'
            self.redirect(url)
        else:
            set_cookie(self.response, "continue", str(url), expires=time.time() + 30 * 86400)
            self.redirect(
...
def main():
    util.run_wsgi_app(webapp.WSGIApplication([
        (r"/facebook/login", LoginHandler),
        (r"/facebook/logout", LogoutHandler),
    ]))
